...
# Usage: get_image_url [machine] [feed]
get_image_url() {
    local machine="${1:-$MACHINE}"
    local feed="${2:-$FEED}"
    echo "${VENUS_UPDATE_BASE_URL}/${feed}/images/${machine}/venus-image-${machine}.wic.gz"
}

# Get the full Docker image tag.
# Usage: get_docker_tag [version] [machine]
get_docker_tag() {
    local version="${1:-$VENUS_VERSION}"
    local machine="${2:-$MACHINE}"
    if [[ "$version" == "latest" ]]; then
        echo "${IMAGE_REGISTRY}/${IMAGE_OWNER}/${IMAGE_NAME}:latest-${machine}"
    else
        echo "${IMAGE_REGISTRY}/${IMAGE_OWNER}/${IMAGE_NAME}:v${version}-${machine}"
    fi
}
